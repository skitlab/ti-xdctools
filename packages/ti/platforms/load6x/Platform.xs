...
function getExecCmd(prog, packagePath)
{
    ...
    if (os == "Solaris" || os == "Linux") {
        return (packagePath + os + "/exec6x -root " + prog.build.target.rootDir + " $(1) " + prog.name);
    }
    if (os == "Windows") {
        if (prog.build.target.isa == "64") {
            return ('"' + packagePath + 'Windows/load6x.exe" -q $(1) "'
                + prog.name + '"');
        }
    }
    
    return ("@$(ECHO) load6x platform package cannot execute " + prog.name +
           " on " + os + "\n");
}
...
/*
 *  @(#) ti.platforms.load6x; 1, 0, 1, 1,116; 8-17-2012 22:16:46; /db/ztree/library/trees/platform/platform-n27x/src/
 */

...
function getExeContext(prog)
{
    ...
    var context = xdc.useModule('xdc.platform.ExeContext');

    /* create a default ExeContext execution context */
    var cpu = context.create(this.$module.CPU, this.$module.BOARD);
    ...
    if (this.$private.args != undefined && 'pmst' in this.$private.args) {
        var mpnmcmask = 0x40;
        if (~this.$private.args.pmst & mpnmcmask) {
            this.externalMemoryMap["EPROG"].len = 0x7000;
        }
    }
...
/*
 *  @(#) ti.platforms.dsk5402; 1, 0, 0,373; 8-17-2012 22:13:54; /db/ztree/library/trees/platform/platform-n27x/src/
 */

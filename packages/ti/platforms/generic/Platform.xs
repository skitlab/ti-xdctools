...
function instance$meta$init(name, args)
{
    ...
    if (this.deviceName == undefined) {
        if (args != null && "deviceName" in args) {
            ...
            this.$module.$logError("Parameter deviceName must be specified "
                + " for 'ti.platforms.generic' platform.", this,
                this.deviceName);       
        }
    }

    if (this.catalogName == undefined) {
        if (args != null && "catalogName" in args) {
            ...
            this.$module.$logError("Parameter catalogName must be specified "
                + " for 'ti.platforms.generic' platform.", this,
                this.deviceName);       
        }
    }
    ...
    if (this.clockRate == undefined) {
        if(args != null && "clockRate" in args) {
            this.clockRate = args.clockRate;
        }
        else {
            this.$module.$logError("Parameter clockRate must be specified "
                + " for 'ti.platforms.generic' platform.", this,
                this.clockRate);        
        }
    }

    if (this.sectMap.length == 0) {
        if (args != null && "sectMap" in args) {
            this.sectMap = args.sectMap;
        }
    }

    if (this.customMemoryMap.length == 0) {
        if (args != null && "customMemoryMap" in args) {
            this.customMemoryMap = args.customMemoryMap;
        }
    }
    
    if (this.renameMap.length == 0) {
        if (args != null && "renameMap" in args) {
            this.renameMap = args.renameMap;
        }
    }

    if (this.externalMemoryMap.length == 0) {
        if (args != null && "externalMemoryMap" in args) {
            this.externalMemoryMap = args.externalMemoryMap;    
        }
    }

    if (this.codeMemory == undefined) {
        if (args != null && "codeMemory" in args) {
            this.codeMemory = args.codeMemory;  
        }
    }

    if (this.dataMemory == undefined) {
        if (args != null && "dataMemory" in args) {
            this.dataMemory = args.dataMemory;  
        }
    }

    if (this.stackMemory == undefined) {
        if (args != null && "stackMemory" in args) {
    ...
    if (!(this.deviceName in xdc.om[this.catalogName])) {
        this.$module.$logError("Device " + args.deviceName + " does not"
            + " exist in " + this.catalogName, this, args.deviceName);
    }
...
/*
 *  @(#) ti.platforms.generic; 1, 0, 0, 1,369; 8-17-2012 22:16:37; /db/ztree/library/trees/platform/platform-n27x/src/
 */

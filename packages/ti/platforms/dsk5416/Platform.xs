...
function getExeContext(prog)
{
    ...
    /* redefine the external memory map based on pmst */
    var ovlymask = 0x20;
    var mpnmcmask = 0x40;
    var ovlybit;
    var mpnmcbit;
  
    if (this.$private.args != undefined && 'pmst' in this.$private.args) {
        ovlybit = this.$private.args.pmst & ovlymask;
        mpnmcbit = this.$private.args.pmst & mpnmcmask;
    }
    else {
        ovlybit = 0;
        mpnmcbit = 0;
    ...
    if (ovlybit != 0) {
        delete this.externalMemoryMap["EPROG1"];
    }
    if (mpnmcbit == 0) {
        this.externalMemoryMap["EPROG0"].len = 0x4000;
        delete this.externalMemoryMap["VECT"];
        delete this.externalMemoryMap["EPROG2"];
    }
...
/*
 *  @(#) ti.platforms.dsk5416; 1, 0, 0,373; 8-17-2012 22:13:56; /db/ztree/library/trees/platform/platform-n27x/src/
 */

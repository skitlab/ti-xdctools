...
function getExecCmd(prog)
{
    ...
    if (os == "Linux") {
        var testScript;
        var args = this.$private.args;
        if (args != null && args.testScript != null) {
            testScript = args.testScript + " " + prog.name;
        } else {
            testScript = xdc.getPackageBase( "ti.platforms.evmDM6446" ) +
                         "testrunner.sh";
        }
        return (testScript +
            ...
            prog.name);
        /* [dm]TODO:M should do something else if not running on Linux; also,
         * is there a better way to pass all 1-... arguments?
         */
     }
...
/*
 *  @(#) ti.platforms.evmTNETV107X; 1, 0, 0, 1,109; 8-17-2012 22:16:17; /db/ztree/library/trees/platform/platform-n27x/src/
 */

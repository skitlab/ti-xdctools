...
function getMemoryMap(regs)
{
    ...
    if (regs != null) {
        if (regs.dnum != undefined) {
            dnum = regs.dnum;
            }
            if (regs.symmetric != undefined) {
                sym = regs.symmetric;
            }
    }
    ...
    if (sym == true) {
        map["L2RAM"] = memBlock["SYMGEML2RAM"];
    }
    else {
        if (dnum == 1) {
                map["L2RAM"] = memBlock["ASYMGEM1L2RAM"];
            }
            else if (dnum == 2) {
                map["L2RAM"] = memBlock["ASYMGEM2L2RAM"];
            }
            else {
                map["L2RAM"] = memBlock["ASYMGEM0L2RAM"];
            }
    }
    
    if (regs != null && "l2Mode" in regs && regs.l2Mode != null) {
        if (this.$module.cacheSizeL2[regs.l2Mode] != undefined) {
            this.memMap["L2RAM"].len -= this.$module.cacheSizeL2[regs.l2Mode]; 
            if (this.memMap["L2RAM"].len == 0) {
                delete this.memMap["L2RAM"];
            }
        }
        else {

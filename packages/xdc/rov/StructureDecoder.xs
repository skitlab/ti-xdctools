...
function _parseTypeString(fieldType)
{
    ...
    switch (fieldType[0]) {
        ...
        case 'U': {
            ...
            decoderPrint("    Field is a scalar (" + fieldType + "), size: " + type.size + 
                         " align: " + type.align);
...
function fetchStruct(structType, addr, addrCheck)
{
    ...
    var buf = this.$private.memReader.readMaus(Number(addr), structType.$sizeof(), addrCheck);
...
function fetchArray(structType, addr, len, isScalar, addrCheck)
{
    ...
    var buf = this.$private.memReader.readMaus(Number(addr), structType.$sizeof() * len, 
                             addrCheck);

...
function run(cmdr, args)
{
    ...
    symInst = SymbolTable.create(controllerIndex);
...
function getTreeTableRecur(treeNode, viewName)
{
    ...
    /* 
     * The properties array is a homogeneous array of structures. Each structure
     * in the array will be converted to a node, and each of the fields in the 
     * structure will become a property of that node.
     *
     * This cannot leverage the 'setNodesChildren' API because it needs to add
     * to the existing list of children.
     */

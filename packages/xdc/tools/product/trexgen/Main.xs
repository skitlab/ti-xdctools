...
function run(cmdr, args)
{
    ...
    */
    ...
    createProjectSpec(opts, opts.outDir, templateInfo);
     ...
     /* create Content.xml */
     var template = xdc.loadTemplate("content.xml.xdt");
    ...
    if (templateInfo.module.templateGroupArr.length > 0) {
        ...
        for each (var element in templateInfo.module.templateGroupArr) {
            ...
            for  each (var path in paths) {
                ...
                var index = 0;
                var projectSpecTemplate = xdc.loadTemplate("projectspecTemplate.projectspec.xdt");
                ...
                for each (var template in templateInfo.module.templateArr) {
                    ...
                    if (addToTRex) {
                        var descriptionArray = templateInfo.descArr[index++];
                        for each (var group in template.groups) {
                            if (group == element.id) {
                                var name = composeDirectoryName(descriptionArray.templateId);
                                projectSpecTemplate.genFile(destinationDir + '/' + name+".projectspec", this, [template, descriptionArray, opts.productSchema.id, templateInfo.module.templateGroupArr]);
                            }
                        }
                    }
                }
...
/*
 *  @(#) xdc.tools.product.trexgen; 1, 0, 0,4; 10-28-2012 21:41:21; /db/ztree/library/trees/xdctools/xdctools-f16x/src/
 */

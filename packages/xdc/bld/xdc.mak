...
#
#	HOSTOS		- equals "Windows" for Windows; otherwise "Solaris" ...
...
#			  the current package.
#
#	FIXLIST		- post-process generated tar include files
#
...
#
#  remove the environment variable ENV from all commands to prevent 
#  shell from sourcing the named file.  This file can only contain 
#  user specific settings that should *not* affect the build.
#
unexport ENV
...
#
#  Error check the definitions of JRE and CLASSPATH.
#
ifeq (0,$(MAKELEVEL))
    ifneq (,$(findstring environment,$(origin JRE)$(origin CLASSPATH)))
       $(warning One of JRE or CLASSPATH is defined by the environment)
    endif
endif
...
# gzip reads the GZIP environment variable for addition options!!!
unexport GZIP

#
#  We make the following overridable because the names of these 
#  utilities may vary (tconf.x5 verses tconf, for example).  To change 
...
unexport TCONF XS CMV CMP TEE SAVEENV PKGTOP FIXDEP FIXLIST FINDPKGS MKDEP TAR XDC ZIP UNZIP

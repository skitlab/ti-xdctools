...
function _mkProg2(pkg, aexe, out)
{
    ...
    if (aexe.$private.noasm == false
        || (acfg != null && acfg.$private.legacyTcf)) {
...
function _mkExe(pkg, aexe, sharedCfg, out)
{
    ...
    var makeFileName = pkg.cfgDir + aexe.$private.name + ".mak";
    out.write("-include " + makeFileName + "\n");
...
function _mkConfig(pkg, acfg, out)
{
    ...
    if (acfg.$private.noasm == false || acfg.$private.legacyTcf) {
    ...
    /* add -Dxdc_cfg__header__=... to compiled cfg object */
...
function _mkSchema(pkg, out)
{
    ...
    if (pkg.$private.executables.length == 0) {
        ...
        /* If there are multiple releases in the package, we don't need them
         * all forcing generation of the schema include file. If we did that,
         * cfginc.js would run multiple times and possibly in parallel.
         */
        for (var r0 in pkg.$private.releases) {
            out.write(pkg.relDir + r0.name + pkg.name.replace(/\./g, '/')
	        + "package/package.rel.xml: package/package.cfg.xdc.inc\n");
            break;
        }

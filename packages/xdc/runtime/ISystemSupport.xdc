...
interface ISystemSupport {
     ...
     *      `{@link System#atexit()}`.  If the application exits via the
     *      ANSI C Standard `exit()` function rather than via System_exit(),
     *      `stat` will be equal to `{@link System#STATUS_UNKNOWN}`.
     *
     *  @see System#atexit
     */
    Void exit(Int stat);

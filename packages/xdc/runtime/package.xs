...
function checkProxies(unit)
{
    ...
    for (var i = 0; i < unit.$$proxies.length; i++) {
        ...
        if (!pud.delegate$.$used) {
            throw new Error("A delegate '" + pud.delegate$.$name + "' is "
                + "assigned to proxy '" + pud.$name + "' too late in the "
                + "configuration. A workaround is to call xdc.useModule('"
...
/*
 *  @(#) xdc.runtime; 2, 1, 0,394; 11-3-2012 15:19:15; /db/ztree/library/trees/xdc/xdc-y44x/src/packages/
 */

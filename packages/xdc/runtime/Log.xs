        ...
        if ('$$logEvtCfgs' in mod && mod.$$logEvtCfgs.length > 0) {
            for each (var cn in mod.$$logEvtCfgs) {
                ...
                if (this.idToInfo[key] != null) {
                    this.$logWarning("Two events with the same id: "
                        + mod.$name + '.' + cn + " and "
                        + this.idToInfo[key].replace("::", " share the msg: '")
                        + "'.  You must reconfigure the message of one of "
                        + "these events to ensure its id will be unique.",
                        this, "idToInfo");
                }
        ...
        if ('$$logEvtCfgs' in mod && mod.$$logEvtCfgs.length > 0) {
            for each (var cn in mod.$$logEvtCfgs) {
                ...
                if (mod.$used && desc.$private.id == null) {
                    this.$logWarning("The event '" + mod.$name + "." + cn + "' has not been assigned an id.  Try adding xdc.useModule('xdc.runtime.Log') to your configuration",
                            mod, cn);
                }

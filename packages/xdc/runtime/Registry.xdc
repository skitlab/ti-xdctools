 ...
 *  @p
 *
 *  All events logged registered modules will be sent to the logger configured
 *  for the `Registry` module. For example, to configure the logger for use by
 *  all modules managed by `Registry`:
 *  @p(code)
 ...
 *  `{@link Diags#ALWAYS_ON Diags.ALWAYS_ON}` will be
 *  permanently on for all modules managed by `Registry`. 
 *
...
module Registry
{
     ...
     *  @p
     */
    Result addModule(Desc *desc, String modName);
     ...
     */
    Desc *findByName(String modName);
     ...
     */
    Desc *findByNamePattern(String namePat, Int len, Desc *prev);
     ...
     */
    Bool getMask(String name, Types.DiagsMask *mask);
     ...
     */
    String getModuleName(Desc *desc);
...
internal:
    ...
    Desc *findByNameInList(String name, Desc *listHead);
    Void newModule(Desc *desc, String modName);
    Bool matchPattern(String pattern, Int len, String modName);

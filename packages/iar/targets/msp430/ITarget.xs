...
function archive(goal)
{
    ...
    if (tool2cmd != null) {
        ...
        result.cmds = _bldUtils.expandString(tool2cmd["ar"], {
            ...
            aropts:	goal.opts,
            files:	goal.files
        }) + "\n" + _bldUtils.expandString(tool2cmd["xlib"], {
            XLIBOPTS_P:	target.xlibOpts.prefix,
            XLIBOPTS_S: target.xlibOpts.suffix
        });
    }
...
function getVersion()
{
    ...
    var raw = getRawVersion.call(this);
...
function _mkCmds(target)
{
    ...
    cmd = prefix + target.vers.cmd + " " + target.vers.opts;
    tool2cmd["vers"] = cmd;

    /* define the xlib command template */
    cmd = prefix + target.xlib.cmd + " $(XLIBOPTS_P) " +  target.xlib.opts
        + " $(XLIBOPTS_S)\n";
    tool2cmd["xlib"] = cmd;
